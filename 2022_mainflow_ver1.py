...
maskLowwerBound = [[  0,188,  0], [  0, 134,  18], [  0,121,121], [  0,188,  0], [ 12,184, 91], [ 86,  0,243], [  0,  0,  0]]
maskUpperBound  = [[ 22,255,255], [ 10, 215, 196], [ 11,255,255], [ 22,255,255], [ 31,255,192], [117,192,255], [179, 95,164]]
...
def maskAll():  #process masks  input: three caps/ output: eleven masked img (sign, pot, tube, 4 colors)
    ...
    maskName['signMask'] = cv2.cvtColor(maskName['signMask'],cv2.COLOR_HSV2BGR)
...
def waterDetect():  #input: four color mask img/ output: loop until water
    contours, hierarchy = cv2.findContours(maskName[list(maskName)[variables['colorCode']]], cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    global motorOutput
    global taskOutput
    for cnt in contours:
    ...
    # maskName[list(maskName)[variables['colorCode']]] = cv2.cvtColor(maskName[list(maskName)[variables['colorCode']]],cv2.COLOR_GRAY2BGR)
    cv2.imshow('result2', maskName[list(maskName)[variables['colorCode']]])
...
def tubeDetect():   #input: mask tube img/ output: (change global variable) motorOutput
    ...
    for i in range(320, 640):
        ...
        #chop the image
        if boundary[2] == 0:
            if (maskName['tubeShow'][upbound][i][0] != num or maskName['tubeShow'][upbound][i][1] != num or maskName['tubeShow'][upbound][i][2] != num):
                if ruCounter > 2:
                    boundary[2] = i
                    cv2.circle(maskName['tubeShow'], (i, upbound), 5, (255, 204, 0), -1)
                else :
                ...
                ruCounter = 0
        if boundary[3] == 0:
            if (maskName['tubeShow'][lowbound][i][0] != num or maskName['tubeShow'][lowbound][i][1] != num or maskName['tubeShow'][lowbound][i][2] != num):
                if rlCounter > 2:
                    boundary[3] = i
                    cv2.circle(maskName['tubeShow'], (i, lowbound), 5, (255, 204, 0), -1)
                else :
